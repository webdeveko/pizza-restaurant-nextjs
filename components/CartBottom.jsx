...
import CheckoutDetais from "./CheckoutDetais";
...
const CartBottom = () => {
  const { setIsOpen, cart, carTotal } = useContext(CartContext);
  // modal state
      ...
      {cart.length >= 1 ? (
        <div className="px-6 py-3 lg:py-6 mt-auto">
          ...
          <div className="flex items-center justify-between mb-6 text-lg font-semibold font-robotoCondensed">
            <div>Total:</div>
            <div>${parseFloat(carTotal).toFixed(2)}</div>
          </div>
          {/* btn */}
        ...
        <div className="absolute top-0 w-full h-full flex justify-center items-center -z-10">
          <div className="font-semibold">Your car is empty</div>
        </div>
      )}
      {/* checkout modal */}
      {modal && (
        ...
        >
          ...
          <CheckoutDetais setModal={setModal} />
        </Modal>
      )}
    </>
  );
